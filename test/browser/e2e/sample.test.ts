...
describe('test text replacer extension with react app', () => {
  ...
  beforeAll(async () => {
    ...
    if (!context.extPage) {
      throw new Error('extPage undefined!');
    }
    if (!context.appPage) {
      throw new Error('appPage undefined!');
    }
    if (!context.browser) {
      throw new Error('browser undefined!');
    }
  ...
  it('should render a button in the web application', async () => {
    expect(extPage).toEqual(extPage);
    expect(appPage).toEqual(appPage);
